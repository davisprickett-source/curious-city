...
interface MobileCuriositiesLayoutProps {
        ...
        <div className="max-w-5xl mx-auto w-full">
          <div className="flex flex-col gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Metadata */}
              {(item.location || (item.sources && item.sources.length > 0) || item.source || item.year) && (
...
export default function MobileCuriositiesLayout({
  curiosities,
  cityName,
  exploreLinks = [],
  footer,
  url
}: MobileCuriositiesLayoutProps) {
  const [scrollProgress, setScrollProgress] = useState(0)
  const [activeSection, setActiveSection] = useState(0)
  const [isDrawerOpen, setIsDrawerOpen] = useState(false)

  // Page ID for ad targeting
  const pageId = `${cityName.toLowerCase().replace(/\s+/g, '-')}-curiosities-mobile`
