    ...
    >
      ...
      <div className="p-4 md:p-6 space-y-3 md:space-y-4">
        ...
        <button
          ...
          className="md:hidden w-full flex items-center justify-center gap-2 py-2 text-sm font-medium text-[#c65d3b] hover:text-[#a84d2f] transition-colors"
        >
        ...
        {/* Expandable details - collapsed on mobile, always visible on desktop */}
        <AnimatePresence initial={false}>
          {(isExpanded || typeof window === 'undefined') && (
            ...
            >
              <MobileExpandedContent spot={spot} hasMultipleLocations={hasMultipleLocations} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="hidden md:block space-y-4">
          <FullSpotContent spot={spot} hasMultipleLocations={hasMultipleLocations} />
        </div>
      </div>
    </div>
  )
...
// Full content shown on desktop (and in mobile expanded state)
function FullSpotContent({ spot, hasMultipleLocations }: { spot: BestOfSpot; hasMultipleLocations: boolean }) {
  return (
    <>
      {/* The Order */}
      {spot.order && (
...
// Compact content for mobile expanded state
function MobileExpandedContent({ spot, hasMultipleLocations }: { spot: BestOfSpot; hasMultipleLocations: boolean }) {
  return (
    <>
      {/* The Order */}
      {spot.order && (
        <div className="border-l-4 border-[#c65d3b] pl-3 py-2 bg-amber-50/50 rounded-r-lg">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold text-[#c65d3b] uppercase tracking-wider">The Order</span>
          </div>
          <p className="text-sm text-neutral-800 font-medium">{spot.order}</p>
        </div>
      )}

      {/* Why - shorter on mobile */}
      ...
      <div className="grid grid-cols-2 gap-2 pt-3 border-t border-neutral-200">
        {spot.address && !hasMultipleLocations && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-3 py-2 bg-neutral-50 hover:bg-neutral-100 rounded-lg transition-colors"
          >
            <svg className="w-4 h-4 text-[#c65d3b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
            </svg>
            <span className="text-xs font-medium text-neutral-700">Directions</span>
          </a>
        )}
        {spot.instagram && (
          <a
            ...
            className="flex items-center gap-2 px-3 py-2 bg-neutral-50 hover:bg-neutral-100 rounded-lg transition-colors"
          >
            <svg className="w-4 h-4 text-[#c65d3b]" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
            </svg>
            <span className="text-xs font-medium text-neutral-700">Instagram</span>
          </a>
        )}
        {spot.website && (
          <a
        ...
        {spot.hours && (
          <div className="flex items-center gap-2 px-3 py-2 bg-neutral-50 rounded-lg">
            ...
            <span className="text-xs font-medium text-neutral-700">{spot.hours}</span>
          </div>
        )}
      </div>
    </>
