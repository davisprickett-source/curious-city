...
export function AnimatedMenuButton({ isOpen, onClick }: AnimatedMenuButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center justify-center w-10 h-10 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-600 focus:ring-offset-2 rounded ${
        isOpen ? 'text-accent-600' : 'text-neutral-900 hover:text-accent-600'
      }`}
