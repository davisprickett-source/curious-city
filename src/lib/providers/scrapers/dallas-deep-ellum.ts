/**
 * Dallas / Deep Ellum venues scraper
 *
 * Deep Ellum is Dallas's live music and arts district.
 * Historic neighborhood with converted warehouses and legendary venues.
 *
 * Venues:
 * - Granada Theater - Historic 1946 venue, 1,200 capacity
 * - Trees - Deep Ellum landmark since 1990, 600 capacity
 * - Club Dada - Intimate club, eclectic booking
 * - Three Links - 300 capacity, punk/indie
 * - The Rustic - 3,000 capacity outdoor venue
 */
...
const VENUES = [
  {
    ...
    url: 'https://www.granadatheater.com/events',
    slug: 'granada-theater',
    relevanceBoost: 15,
  },
  {
    name: 'Trees',
    url: 'https://treesdallas.com/events',
    slug: 'trees',
    relevanceBoost: 15,
  },
  {
    name: 'Club Dada',
    url: 'https://clubdada.com/events',
    slug: 'club-dada',
    relevanceBoost: 10,
  },
  {
    name: 'Three Links',
    url: 'https://threelinksdeepellum.com/events',
    slug: 'three-links',
    relevanceBoost: 10,
  },
]

export class DallasVenuesProvider extends BaseScraperProvider {
  constructor() {
    super({
      name: 'Dallas Deep Ellum Venues',
  ...
  async fetchEvents(citySlug: string, options?: FetchOptions): Promise<ProviderResult> {
    ...
    if (citySlug !== 'dallas') {
      return this.createEmptyResult('Dallas Venues only supports Dallas')
    }
  ...
  private parseJsonLdEvent(
    ...
    const description = item.description || `${name} at ${venueName} in Deep Ellum.`
    ...
    const tags = ['music', 'live', 'concert', 'deep-ellum']
    if (venue.slug === 'club-dada' || venue.slug === 'three-links') {
      tags.push('intimate')
    }

    return {
      source: 'venue-scraper',
      sourceId: `dallas-${venue.slug}-${eventId}`,
      title: name,
  ...
  private parseHtmlEvents(
    ...
    while ((match = eventPattern.exec(html)) !== null) {
      ...
      events.push({
        ...
        sourceId: `dallas-${venue.slug}-${eventId}`,
        title,
        description: `${title} at ${venue.name} in Deep Ellum.`,
        url: fullUrl,
        startDate,
        venue: venue.name,
        city: 'Dallas',
