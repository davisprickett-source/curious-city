...
const SKIP_PATTERNS = [
  ...
  'Thumbs.db',
  '.gitkeep',
  'sequences' // Already being uploaded by the other script
];

// Get all files in directory, excluding skip patterns
function getAllFiles(dirPath, arrayOfFiles = []) {
  ...
  files.forEach((file) => {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      ...
      // Only upload images and common web assets
...
async function uploadFile(filePath) {
  ...
  const command = new PutObjectCommand({
    ...
    CacheControl: 'public, max-age=31536000, immutable', // Cache images for 1 year
  });
